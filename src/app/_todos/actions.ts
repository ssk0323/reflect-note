...
// --------------------------------------------------------------------------
// Move to arbitrary (bucket, position) (Issue #42)
// --------------------------------------------------------------------------

/** B-1a の「タップで掴む → 行間スロットを挿入」を支える server action。
 *  move_todo RPC (migration 0014) で position の再採番を atomic に行う。
 *  bucket 跨ぎの移動と任意 position への挿入をどちらも扱う。 */
export async function moveTodo(
  ...
  // 23505 (DEFERRABLE UNIQUE 衝突) や 40P01 (deadlock) は並列 RPC で発生し得るので
  // 数回 retry する (reorderTodo / tryInsertWithPosition と同じパターン)。
  const MAX_RETRIES = 4;
  let lastErr: unknown = null;
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
